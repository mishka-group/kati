defmodule Kati.Theme.Palette do
  ...
  **This module stores no mode of its own.** `mode/0` reads the theme
  ...
  @tokens [
    # ── Surfaces ─────────────────────────────────────────────────────────
    {:paper, 0xFFEFECE7, 0xFF121110, :drawn, "The page behind everything, and the ground every contrast is measured against."},
    {:card, 0xFFFBFAF8, 0xFF1E1D1B, :drawn, "A card lifted off the page — by a shadow in light, by a hairline in dark."},
    ...
    {:bar_neutral, 0xFFD8D2C8, 0xFF3D3935, :ramp, "A bar in a chart that is not the highlighted one."},
    {:star_empty, 0xFFE0DAD1, 0xFF2A2826, :ramp, "An unfilled star in a rating — the faintest mark the design draws."},

    # ── Ink and neutral marks ────────────────────────────────────────────
    {:ink, 0xFF1A1917, 0xFFF5F2EE, :drawn, "Headline and title ink, the strongest thing on the page."},
    {:ink_soft, 0xFF5C574F, 0xFFCBC7C1, :ramp, "Body copy — one step below ink, for text meant to be read rather than scanned."},
    ...
    {:settled_ink, 0xFF9C958B, 0xFF79736D, :ramp, "The title of a row already done or already watched."},
    ...
    {:rail_idle, 0xFFC4BDB3, 0xFF4A453F, :drawn, "The 3pt rail beside a timeline row that is not now."},

    # ── On an ink-filled control (which inverts in dark) ──────────────────
    {:on_ink, 0xFFFBFAF8, 0xFF1A1917, :drawn, "A label on an ink fill, which inverts with the mode rather than following it."},
    {:on_ink_glyph, 0xFFF5F2EE, 0xFF1A1917, :inversion, "A glyph on an ink card, where the ground is already inverted."},
    {:on_ink_muted, 0xFFBFB8AC, 0xFF4A4238, :inversion, "The muted step on an ink fill — a day name under its date, say."},
    {:on_ink_meta, 0xFF8A837B, 0xFF6E6860, :inversion, "The mono meta step on an ink fill."},
    {:on_ink_count, 0x99FBFAF8, 0x991A1917, :alpha, "A count inside a selected, ink-filled chip."},
    {:on_ink_count_soft, 0xA6FBFAF8, 0xA61A1917, :alpha, "The same count, one screen's softer alpha."},
    ...
    {:count_idle_soft, 0xA65C574F, 0xA6CBC7C1, :alpha, "The unselected twin of `on_ink_count_soft`."},
    {:count_idle_faint, 0x8C5C574F, 0x8CCBC7C1, :alpha, "The faintest unselected count, on the screen that draws it lightest."},
    {:ink_fill, 0xFF1A1917, 0xFFF7EFE4, :drawn, "The fill of the hero's call-to-action pill."},

    # ── Cream ────────────────────────────────────────────────────────────
    {:cream_ink, 0xFF1A1917, 0xFFF7EFE4, :drawn, "The headline on a cream card. Screen 28 makes the CTA pill this same colour."},
    {:cream_body, 0xFF4A4238, 0xFFE4DBCE, :ramp, "A note paragraph on cream — the longest text the design sets on a warm ground."},
    ...
    {:cream_raise, 0x99FFFFFF, 0xFF3A342D, :drawn, "A chip or patch lifted a step off the cream card."},
    ...
    {:gold_icon, 0xFFC98A3E, 0xFFC98A3E, :hue, "A gold glyph on cream — the note/annotation hue."},
    {:gold_label, 0xFFC08A4C, 0xFFC08A4C, :hue, "A gold mono label — a clash count, a warning line above a row."},
    {:gold_text, 0xFF96723C, 0xFFC98A3E, :step, "Gold text on a wash — shaded for paper, unshaded for near-black."},
    ...
    {:accent_wash, 0x2EE8823C, 0x2EE8823C, :hue, "An 18% orange wash behind a highlighted token."},
    {:accent_fill, 0x47E8823C, 0x47E8823C, :hue, "A stronger orange fill behind a matched search run."},
    {:green, 0xFF4E9A73, 0xFF4E9A73, :hue, "Done, ticked, live. A hue, so it does not move with the ground."},
    {:green_wash, 0x294E9A73, 0x294E9A73, :hue, "The ground of a live status pill."},
    {:green_text, 0xFF3E8460, 0xFF4E9A73, :step, "Green text on the green wash — shaded for paper, unshaded for near-black."},
    {:red, 0xFFB4553C, 0xFFB4553C, :theme, "Destructive, stale, over. `Kati.Theme.dark/0` keeps `error: @red`."},
    ...
    {:bronze, 0xFFB08E55, 0xFFB08E55, :theme, "Meals, money. `Kati.Theme.bronze/0` takes no mode."},
    ...
    {:hairline_strong, 0x1A1A1917, 0x1AF5F2EE, :alpha, "A 10% rule where a group needs more separation."},
    {:border_soft, 0x241A1917, 0x24F5F2EE, :alpha, "A 14% outline, the lightest border the design draws."},
    {:border, 0x291A1917, 0x29F5F2EE, :alpha, "The 16% outline on an outlined control."},
    {:border_strong, 0x2E1A1917, 0x2EF5F2EE, :alpha, "An 18% outline, on a control that needs to read as pressable."},
    {:border_stronger, 0x331A1917, 0x33F5F2EE, :alpha, "A 20% outline on a small subtle disc."},
    {:track_ink, 0x381A1917, 0x38F5F2EE, :alpha, "A 22% ink track, used where a card is already pale."},
    {:divider_heavy, 0x591A1917, 0x59F5F2EE, :alpha, "A 35% vertical divider between two numbers."},
    ...
    {:fab_fill, 0xFF1A1917, 0xFFF5F2EE, :drawn, "The add button beside the dock — ink on paper, and paper on ink in dark."},
    ...
    {:transparent, 0x00FFFFFF, 0x00FFFFFF, :hue, "Nothing drawn, in either mode. An explicit absence rather than an omission."},

    # ── Over a photograph ────────────────────────────────────────────────
    {:on_media, 0xFFFBFAF8, 0xFFFBFAF8, :media, "A title burnt over artwork, where the ground is a photograph."},
    {:on_media_meta, 0xBFFBFAF8, 0xBFFBFAF8, :media, "Mono meta burnt over artwork, at 75% so the picture still reads through."},
    {:on_media_track, 0x40FBFAF8, 0x40FBFAF8, :media, "A progress track burnt into the bottom edge of artwork."},
    ...
    {:lock_ink_65, 0xA6FFFFFF, 0xA6FFFFFF, :media, "A lock-screen widget's second line, at 65% over the wallpaper."},
    {:lock_ink_60, 0x99FFFFFF, 0x99FFFFFF, :media, "A lock-screen widget's mono meta line, at 60% over the wallpaper."},
    {:lock_ink_55, 0x8CFFFFFF, 0x8CFFFFFF, :media, "A lock-screen widget's eyebrow — mono, letter-spaced, half-alpha, no accent dash."},
    {:lock_ink_50, 0x80FFFFFF, 0x80FFFFFF, :media, "A lock-screen widget's smallest mono figure, at the edge of legible."},
    {:lock_ink_40, 0x66FFFFFF, 0x66FFFFFF, :media, "The rail beside a lock-screen row that is not now."},
    ...
    {:lock_glass, 0x801C1A18, 0x801C1A18, :media, "The glass panel itself, half-alpha over the wallpaper."},
    {:lock_ground, 0xFF1C1A18, 0xFF1C1A18, :media, "What stands in for the wallpaper when there is no image."}
  ]
  ...
  """
  @spec tokens() :: [%{name: atom(), light: pos_integer(), dark: pos_integer(), source: atom(), meaning: String.t()}]
  def tokens do
