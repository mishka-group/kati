defmodule Kati.Screens.Inbox do
  ...
  **Coming up** is one card of dated rows, because it is a schedule and the
  ...
  # and carries the Mark all action opposite it.
  @doc false
  def mark_all do
    ~MOB"""
  ...
  def out_now(inbox) do
    ~MOB"""
    <Column fill_width={true}>
      {Enum.map(inbox.out_now, fn row -> Kati.Screens.Inbox.release_row(row) end)}
      <Spacer size={26} />
    </Column>
    """
  end
    ...
    <Column fill_width={true}>
      <Row fill_width={true} align="center" padding_top={14} padding_bottom={14}>
        ...
        <Column weight={1.0}>
          ...
          <Spacer size={3} />
          <Text text={row.line} text_size={12} text_color={0xFF8A8479} max_lines={1} />
        </Column>
        <Spacer size={12} />
        <Text text={row.meta} font_family="mono" text_size={10.5} text_color={0xFFB3ACA2} max_lines={1} />
      </Row>
