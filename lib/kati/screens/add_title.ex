defmodule Kati.Screens.AddTitle do
  ...
  **This should eventually be a native bottom sheet**, not a pushed screen:
  ...
  `Kati.Media.Tmdb.search/1`, debounced through `Kati.Media.SearchDebounce`.
  The sheet opens empty (or already searching, when a push hands it a query)
  and never shows board 06's four fixtures to a reader. Each result is shaped
  by `row/1`: `FILM` / `SERIES` from TMDB's kind, the year and overview from
  the result, `added` from the reader's own shelf through `already_added/1`.

  A row's poster is TMDB's path, resolved by `thumb/1` through
  ...
  *Everything here is started*, and a bare one-word msgid beside two long
  ...
  @doc false
  def thumb(r) do
    case Kati.Media.Artwork.local(r[:seed]) || Kati.Media.Artwork.thumbnail(r[:seed]) do
      nil ->
