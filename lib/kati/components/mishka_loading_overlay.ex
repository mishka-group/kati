defmodule Kati.Components.MishkaLoadingOverlay do
  @moduledoc """
  ...
  defp scrim(props, content, ctx) do
    ...
    # `corner_radius` has no default — the scrim is square unless the caller
    # matches it to the region it covers. Merged rather than interpolated: an
    # inline `corner_radius={nil}` still lands in the props map, and `:json`
    # encodes an atom as a string, so the bridge received the radius token "nil".
    ~MOB"""
    <Box fill_width={true} fill_height={true} align={:center} background={fill}>
      {body(props, content)}
    </Box>
    """
    |> put(:corner_radius, radius)
    |> put(:on_tap, absorb(ctx))
  end
