defmodule Kati.Components.MishkaTree do
  @moduledoc """
  ...
  *is* data — a directory listing, a category table — `Enum.map/2` into `nodes`
  ...
  alias Kati.Components.MishkaCheckbox
  ...
  `:has_children`.
      ...
      iex> Kati.Components.MishkaTree.node("mix.exs", "mix.exs", icon: "📄")
      %{label: "mix.exs", value: "mix.exs", icon: "📄"}

      iex> Kati.Components.MishkaTree.node("lib", "lib",
      ...>   children: [Kati.Components.MishkaTree.node("app.ex", "lib/app.ex")])
  ...
  shorthand the web attr takes.
      ...
      iex> Kati.Components.MishkaTree.visible(nodes, [])
      [{%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}, 0, true, false}]

      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.visible(nodes, ["a"]) |> Enum.map(&elem(&1, 0).value)
      ["a", "b"]

      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.visible(nodes, :all) |> Enum.map(&elem(&1, 0).value)
      ["a", "b"]
  ...
  applies to.
      ...
      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.subtree_values(nodes, "a")
      ["a", "b"]

      iex> Kati.Components.MishkaTree.subtree_values([], "a")
      []
  """
  ...
  only some of its descendants are checked.
      ...
      iex> Kati.Components.MishkaTree.check_state(nodes, "a", ["b"])
      :indeterminate

      iex> nodes = [%{label: "a", value: "a", children: [
      ...>   %{label: "b", value: "b"}, %{label: "c", value: "c"}]}]
      iex> Kati.Components.MishkaTree.check_state(nodes, "a", ["b", "c"])
      :checked

  A leaf is simply checked or not:

      iex> Kati.Components.MishkaTree.check_state([%{label: "b", value: "b"}], "b", ["b"])
      :checked
  ...
  the prop inert — the tick landed in the list and the box stayed empty.
      ...
      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.check_state(nodes, "a", ["a"], strictly: true)
      :checked

      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.check_state(nodes, "a", ["a"])
      :unchecked
  """
  ...
  node.
      ...
      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.toggle_check(nodes, "a", [])
      ["a", "b"]

      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.toggle_check(nodes, "a", ["a", "b"])
      []

      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.toggle_check(nodes, "a", [], strictly: true)
      ["a"]
  ...
  reads as unchecked forever and every tap tries to check it again.
      ...
      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.toggle_check(nodes, "a", ["a"], strictly: true)
      []
  """
  ...
  `has_children: true` and has not fetched them yet.
      ...
      iex> Kati.Components.MishkaTree.expanded_values(nodes)
      ["a", "c"]
  """
  ...
  Expand or collapse `value` in an expanded list.
      ...
      iex> Kati.Components.MishkaTree.toggle_expand("a", [])
      ["a"]

      iex> Kati.Components.MishkaTree.toggle_expand("a", ["a", "b"])
      ["b"]
  ...
  with `expanded_values/1` first, so the tap really does close a branch.
      ...
      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]}]
      iex> Kati.Components.MishkaTree.toggle_expand("a", :all, nodes)
      []
  """
  ...
  same "tap it again to put it back" the disclosure has:
      ...
      iex> Kati.Components.MishkaTree.toggle_select("a", [])
      ["a"]

      iex> Kati.Components.MishkaTree.toggle_select("b", ["a"])
      ["b"]

      iex> Kati.Components.MishkaTree.toggle_select("a", ["a"])
      []

  With `multiple: true` a tap adds or removes, and the rest stays:

      iex> Kati.Components.MishkaTree.toggle_select("b", ["a"], multiple: true)
      ["a", "b"]

      iex> Kati.Components.MishkaTree.toggle_select("a", ["a", "b"], multiple: true)
      ["b"]
  """
  ...
  the middle of a range rather than joining it.
      ...
      iex> Kati.Components.MishkaTree.select_range(nodes, ["a"], "b", "d")
      ["b", "c", "d"]

      iex> nodes = [%{label: "a", value: "a", children: [
      ...>   %{label: "b", value: "b"}, %{label: "c", value: "c"}]}, %{label: "d", value: "d"}]
      iex> Kati.Components.MishkaTree.select_range(nodes, ["a"], "d", "b")
      ["b", "c", "d"]
  ...
  be dragged across:
      ...
      iex> nodes = [%{label: "a", value: "a", children: [%{label: "b", value: "b"}]},
      ...>   %{label: "d", value: "d"}]
      iex> Kati.Components.MishkaTree.select_range(nodes, [], "a", "d")
      ["a", "d"]

  With no anchor yet — nothing selected — a hold is just a selection of one:

      iex> Kati.Components.MishkaTree.select_range([%{label: "a", value: "a"}], [], nil, "a")
      ["a"]
  """
  ...
  The test tag on a whole row — the tap and long-press target.
      ...
      iex> Kati.Components.MishkaTree.row_tag("files", "lib/app.ex")
  ...
  The test tag on a row's disclosure.
      ...
      iex> Kati.Components.MishkaTree.toggle_tag("files", "lib")
      "files-toggle-lib"
  """
  ...
  The test tag on a row's checkbox.
      ...
      iex> Kati.Components.MishkaTree.check_tag("files", "lib")
      "files-check-lib"
  """
  ...
  its own tag instead.
      ...
      iex> Kati.Components.MishkaTree.state_tag("files", "lib", "open")
      "files-lib-open"
  """
