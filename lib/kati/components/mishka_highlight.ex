defmodule Kati.Components.MishkaHighlight do
  @moduledoc """
  ...
  `Text`, matched runs through `Kati.Components.MishkaMark`.
  ...
  `Kati.Components.MishkaOverflowList` makes with `visible`. Leave it unset
  ...
  `Kati.Components.MishkaMark` and `Text` directly; that is what the mark is
  for, and the web component draws the same line (it has one `mark_class`).
  """

  import Mob.Sigil

  alias Kati.Components.MishkaMark
  ...
  searched for.
      ...
      iex> Kati.Components.MishkaHighlight.split("Mishka Chelekom", "chel")
      [{:text, "Mishka "}, {:mark, "Chel"}, {:text, "ekom"}]

      iex> Kati.Components.MishkaHighlight.split("This and THIS", "this")
      [{:mark, "This"}, {:text, " and "}, {:mark, "THIS"}]

      iex> Kati.Components.MishkaHighlight.split("This and THIS", "this", case_sensitive: true)
      [{:text, "This and THIS"}]

      iex> Kati.Components.MishkaHighlight.split("one two", [])
      [{:text, "one two"}]

      iex> Kati.Components.MishkaHighlight.split("one two", ["", nil])
      [{:text, "one two"}]
  """
  ...
  invisible, rather than being carried down as an indent.
      ...
      iex> alias Kati.Components.MishkaHighlight
