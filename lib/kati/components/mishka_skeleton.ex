defmodule Kati.Components.MishkaSkeleton do
  @moduledoc """
  ...
  **Ordinary nodes cannot.** `Box`, `Text`, `Row` and `Column` expose no opacity,
  ...
  indicator, which `Kati.Components.MishkaProgress` wraps. (Android imports
  ...
  **`Mob.UI.gpu_view/1` can.** It is a fragment-shader surface — GLES 3.0 on
  ...
  rather than "content goes here", `MishkaProgress` and
  `Kati.Components.MishkaLoadingOverlay` already animate for free.
  ...
  paragraph does not end flush with the margin.
      ...
      iex> Kati.Components.MishkaSkeleton.shares(3, 0.6)
      [1.0, 1.0, 0.6]

      iex> Kati.Components.MishkaSkeleton.shares(1, 0.6)
      [0.6]

  A single full-width bar is `last_line: 1.0`:

      iex> Kati.Components.MishkaSkeleton.shares(1, 1.0)
      [1.0]

  Nonsense counts render nothing rather than raising:

      iex> Kati.Components.MishkaSkeleton.shares(0, 0.6)
      []
  """
