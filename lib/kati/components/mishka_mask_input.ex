defmodule Kati.Components.MishkaMaskInput do
  @moduledoc """
  ...
  **significant characters only**, discarding literals before re-laying them out,
  ...
  the user.
      ...
      iex> Kati.Components.MishkaMaskInput.apply_mask("5551234567", "(999) 999-9999")
      "(555) 123-4567"

      iex> Kati.Components.MishkaMaskInput.apply_mask("(555) 123-4567", "(999) 999-9999")
      "(555) 123-4567"

      iex> Kati.Components.MishkaMaskInput.apply_mask("555", "(999) 999-9999")
      "(555"

      iex> Kati.Components.MishkaMaskInput.apply_mask("abc1234", "aaa-9999")
      "abc-1234"

      iex> Kati.Components.MishkaMaskInput.apply_mask("", "99/99")
      ""
  """
  ...
  The characters a mask would keep from a value — the unformatted payload.
      ...
      iex> Kati.Components.MishkaMaskInput.strip("(555) 123-4567")
      "5551234567"
  """
  ...
  literals included, since that is what the field displays.
      ...
      iex> Kati.Components.MishkaMaskInput.max_length("99/99/9999")
      10

      iex> Kati.Components.MishkaMaskInput.max_length(nil)
      0

  This is handed to the field as `max_length` so the keystroke past the end is
  refused where it happens, rather than accepted and corrected a round trip
  later. `apply_mask/2` still truncates — the two are complementary — but a mask
  that only truncated let a date field accept a nine-digit year, because the
  corrected value was equal to the previous one and the native side treated it
