defmodule Kati.Components.MishkaOverflowList do
  @moduledoc """
  ...
  the same bug `Kati.Components.MishkaPill` documents. iOS's `MobBox` never
  ...
  roughly 7.6dp per character plus padding per token.
      ...
      iex> alias Kati.Components.MishkaOverflowList
  ...
  The test tag on the "+N" counter, given the list's `id`.
      ...
      iex> Kati.Components.MishkaOverflowList.counter_id("langs")
      "langs-counter"
  """
  ...
  tagged, so counting these tags counts what survived the split.
      ...
      iex> Kati.Components.MishkaOverflowList.item_id("langs", 2)
      "langs-item-2"
  """
  ...
  the whole list — the web component makes the same guarantee.
      ...
      iex> Kati.Components.MishkaOverflowList.split([1, 2, 3, 4, 5], %{visible: 2})
      {[1, 2], [3, 4, 5]}

      iex> Kati.Components.MishkaOverflowList.split([1, 2], %{visible: 5})
      {[1, 2], []}

      iex> Kati.Components.MishkaOverflowList.split([1, 2, 3], %{visible: 0})
      {[1], [2, 3]}
  """
