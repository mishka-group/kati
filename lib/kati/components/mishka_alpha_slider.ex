defmodule Kati.Components.MishkaAlphaSlider do
  @moduledoc """
  ...
  `Kati.Components.MishkaSlider` for why.
  """

  import Mob.Sigil

  alias Kati.Components.{Color, Event}
  ...
  The alpha slider.
      ...
      <MishkaAlphaSlider value={@alpha} color="#3b82f6" on_change={:alpha} />

  The track itself is the control — there is no `<Slider>` under it. `phase` is
  `"began"` on touch-down (so a tap sets the alpha), `"dragging"` while the
  finger moves, `"ended"` on release.
  """
  ...
  @doc """
  The alpha a touch at `x` selects, `x` in canvas units from the track's left edge.

  The inverse of the marker, so finger and marker cannot separate.

      iex> Kati.Components.MishkaAlphaSlider.alpha_at(150, 300)
      50.0

      iex> Kati.Components.MishkaAlphaSlider.alpha_at(300, 300)
      100.0

      iex> Kati.Components.MishkaAlphaSlider.alpha_at(-40, 300)
      0.0
  """
  @spec alpha_at(number(), number()) :: float()
  def alpha_at(x, width \\ @width)
  def alpha_at(_x, width) when not is_number(width) or width <= 0, do: 0.0
  def alpha_at(x, width), do: Color.clamp(x, 0, width) / width * 100
  ...
  the position marker.
      ...
      iex> ops = Kati.Components.MishkaAlphaSlider.track(32, 16, {255, 0, 0}, 50)
  ...
  defp marker(width, height, alpha) do
    # Half the 3px stroke, not 2 — with a finger on the track the marker must
    # not sit visibly short of the edge the user is aiming at.
    x = Color.clamp(alpha / 100 * width, 1.5, width - 1.5)
