defmodule Kati.Components.MishkaNavLink do
  @moduledoc """
  ...
  | `href` | string | `nil` | Rides along with `on_tap`, as in `Kati.Components.MishkaAnchor`. |
  ...
  `Kati.Components.MishkaAnchor` already follows). Nor `class` /
  ...
  defp labels(props, active?, disabled?, id) do
    ...
    # column of letters.
    # Every test tag goes on through `put/3`, the rule the list-glyph clause
    # below already follows: a link with no `id` derives nil part tags, an
    # inline `id={nil}` still lands in the props map, and `:json` encodes an
    # atom as a string — so an untagged link reached the bridge carrying the
    # testTag "nil", which every other untagged link also answered to.
    text =
      ~MOB"""
      <Text text={label} text_size={:base} text_color={ink} font_weight={font_weight} max_lines={1} />
      """
      |> put(:id, label_tag)
  ...
  defp trailing(_props, true, opened?, id) do
    ...
    ~MOB(<Text text={chevron} text_size={:sm} text_color={:muted} />)
    |> put(:id, chevron_tag)
  end
  ...
  defp glyph(text, size, tag) when is_binary(text) do
    ~MOB(<Text text={text} text_size={size} text_color={:muted} />)
    |> put(:id, tag)
  end
