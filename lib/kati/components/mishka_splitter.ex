defmodule Kati.Components.MishkaSplitter do
  @moduledoc """
  ...
  riding on the divider would be a ruler sliding under the finger.
      ...
      iex> alias Kati.Components.MishkaSplitter
  ...
  The two pane sizes in dp, clamped into `min..max` so a pane can never vanish.
      ...
      iex> Kati.Components.MishkaSplitter.sizes(%{value: 50, extent: 300})
      {150.0, 150.0}

      iex> Kati.Components.MishkaSplitter.sizes(%{value: 25, extent: 400})
      {100.0, 300.0}

  A value outside the bounds is pulled back inside them, not honoured:

      iex> Kati.Components.MishkaSplitter.sizes(%{value: 0, extent: 300, min: 20})
      {60.0, 240.0}
  """
  ...
  The split percentage after clamping — what the control and the panes agree on.
      ...
      iex> Kati.Components.MishkaSplitter.split(%{value: 95, max: 90})
      90
  """
  ...
  widths, and neither is readable without a handle.
      ...
      iex> Kati.Components.MishkaSplitter.pane_id("panes", 1)
      "panes-pane-1"
  """
  ...
  device test has on the thing it needs to drag.
      ...
      iex> Kati.Components.MishkaSplitter.grip_id("panes")
      "panes-grip"
  """
