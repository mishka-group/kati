defmodule Kati.Components.Color do
  @moduledoc """
  ...
  `#ff0000` and not `#f00000`.
      ...
      iex> Kati.Components.Color.parse("#3b82f6")
      {:ok, {59, 130, 246}}

      iex> Kati.Components.Color.parse("F00")
      {:ok, {255, 0, 0}}

      iex> Kati.Components.Color.parse("#12345")
      :error

      iex> Kati.Components.Color.parse(nil)
      :error
  """
  ...
  every keystroke.
      ...
      iex> Kati.Components.Color.valid?("#fff")
      true

      iex> Kati.Components.Color.valid?("#ff")
      false
  """
  ...
  Format `{r, g, b}` as a lowercase `#rrggbb` string.
      ...
      iex> Kati.Components.Color.hex({59, 130, 246})
      "#3b82f6"

      iex> Kati.Components.Color.hex({0, 0, 0})
      "#000000"
  """
  ...
  `alpha` is a percentage, `0..100`, to match the alpha slider.
      ...
      iex> Kati.Components.Color.argb({255, 0, 0})
      0xFFFF0000

      iex> Kati.Components.Color.argb({255, 0, 0}, 50)
      0x80FF0000
  """
  ...
  Convert HSV to `{r, g, b}`. Hue wraps; saturation and value clamp to `0..100`.
      ...
      iex> Kati.Components.Color.hsv_to_rgb(0, 100, 100)
      {255, 0, 0}

      iex> Kati.Components.Color.hsv_to_rgb(120, 100, 100)
      {0, 255, 0}

      iex> Kati.Components.Color.hsv_to_rgb(240, 100, 100)
      {0, 0, 255}

      iex> Kati.Components.Color.hsv_to_rgb(0, 0, 100)
      {255, 255, 255}

      iex> Kati.Components.Color.hsv_to_rgb(0, 0, 0)
      {0, 0, 0}

  A hue of 380° is a hue of 20°:

      iex> Kati.Components.Color.hsv_to_rgb(380, 100, 100) ==
      ...>   Kati.Components.Color.hsv_to_rgb(20, 100, 100)
      true
  """
  ...
  two as percentages, all rounded.
      ...
      iex> Kati.Components.Color.rgb_to_hsv({255, 0, 0})
      {0, 100, 100}

      iex> Kati.Components.Color.rgb_to_hsv({0, 128, 0})
      {120, 100, 50}

      iex> Kati.Components.Color.rgb_to_hsv({255, 255, 255})
      {0, 0, 100}

  Grey has no hue to speak of, so it reports 0 rather than an arbitrary angle:

      iex> Kati.Components.Color.rgb_to_hsv({128, 128, 128})
      {0, 0, 50}
  """
  ...
  paints, and the base a saturation/value area is built from.
      ...
      iex> Kati.Components.Color.hue_rgb(0)
      {255, 0, 0}

      iex> Kati.Components.Color.hue_rgb(240)
      {0, 0, 255}
  """
  ...
  Hex straight from HSV — the round trip a picker performs on every drag.
      ...
      iex> Kati.Components.Color.hsv_to_hex(217, 76, 96)
      "#3b82f5"
  ...
  half-typed field never crashes a render.
      ...
      iex> Kati.Components.Color.hex_to_hsv("#ff0000")
      {0, 100, 100}

      iex> Kati.Components.Color.hex_to_hsv("nope")
      {0, 0, 100}
  """
  ...
  its readout legible as the swatch moves from black to white.
      ...
      iex> Kati.Components.Color.light?({255, 255, 255})
      true

      iex> Kati.Components.Color.light?({0, 0, 128})
      false
  """
  ...
  Ink that stays readable on `rgb` — near-black on light colours, white on dark.
      ...
      iex> Kati.Components.Color.ink_on({255, 255, 255})
      0xFF111827

      iex> Kati.Components.Color.ink_on({0, 0, 0})
      0xFFFFFFFF
  """
  ...
  Wrap a hue into `0..360`.
      ...
      iex> Kati.Components.Color.wrap_hue(380)
      20.0

      iex> Kati.Components.Color.wrap_hue(-10)
      350.0
  """
  ...
  Clamp a number into `[min, max]`.
      ...
      iex> Kati.Components.Color.clamp(120, 0, 100)
      100
  """
