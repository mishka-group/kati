defmodule Kati.Components.MishkaPopover do
  @moduledoc """
  ...
  `Kati.Components.Anchored` is what replaced it. The trigger is child `[0]`
  ...
  default, which is how `Kati.Components.MishkaMenu` — passing none, and
  ...
  **long press**, the same touch equivalent
  `Kati.Components.MishkaContextMenu` uses for the web's right-click. The
  ...
  whatever owns the screen root — use `Kati.Components.MishkaDialog` or
  `Kati.Components.MishkaDrawer` when the page must be blocked),
  ...
  def panel(props \\ %{}, content \\ []) do
    props = Map.new(props)
    # `nil` means "not given": with an explicit width the panel hugs it (the
    # prop is dropped rather than pinned to a boolean), and without one it
    # fills. `Map.get/3` applies that only when the key is ABSENT, so a props
    # map carrying `fill_width: nil` used to skip the "no width, so fill" half
    # and hug instead. `||` cannot serve here — `false` is a real value, and
    # so is the computed `nil`.
    fill? =
      case Map.get(props, :fill_width) do
        nil -> if Map.get(props, :width), do: nil, else: true
        given -> given
      end
