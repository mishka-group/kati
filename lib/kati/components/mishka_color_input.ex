defmodule Kati.Components.MishkaColorInput do
  @moduledoc """
  ...
  (`Kati.Components.Anchored`), which
  `Kati.Components.MishkaPopover` is built on. The picker does not use it,
  ...
  through typing (see the note on `Kati.Components.Color.hsv_to_hex/3`).
  ...
  | `on_toggle` | event tag | — | `{:tap, tag}` on the ▾ trigger **and on the swatch**. |
  | `on_hue` | event tag | — | Forwarded to the panel's hue strip: `{:drag, tag, %{x:}}`. Convert with `MishkaHueSlider.hue_at/2`. |
  | `on_area` | event tag | — | Forwarded to the panel's square: `{:drag, tag, %{x:, y:}}`. Convert with `MishkaColorPicker.sv_at/4`. |
  ...
  alias Kati.Components.{Color, Event, MishkaColorPicker}
  ...
  def color_input(props \\ %{}) do
    ...
    <Column fill_width={true}>
      ...
      <Row fill_width={true} align={:center}>
        {swatch_box(swatch, props, disabled?)}
        <Spacer size={8} />
        {field}
        <Spacer size={8} />
  ...
  defp trigger(props, disabled?) do
    ...
    # fill_width={false}: a Box given neither width nor fill_width fills its
    # parent, and this trigger sits in a Row beside a filling TextField — the
    # two then fight for the width. Same line the pill, the mark and the tree's
    # disclosure arrow all needed.
    ~MOB"""
    <Box
  ...
  # The swatch opens the panel too. It is the most colour-like thing on the row,
  # so it is what a finger goes for — leaving it inert meant the ▾ trigger was
  # the only way in, and nothing about the swatch said so.
  defp swatch_box(fill, props, disabled?) do
    node = ~MOB"""
    <Box
      width={30}
      height={30}
    ...
    put(node, :on_tap, if(disabled?, do: nil, else: Event.handler(Map.get(props, :on_toggle))))
  end
  ...
  does not — which is the normal state for most of the time someone is typing.
      ...
      iex> Kati.Components.MishkaColorInput.commit("#ff0000")
      {:ok, {0, 100, 100}}

      iex> Kati.Components.MishkaColorInput.commit("#ff00")
      :incomplete
  """
