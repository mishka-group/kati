defmodule Kati.Components.MishkaContextMenu do
  @moduledoc """
  Native Mob port of Mishka Chelekom's **headless Context Menu** — the actions
  for a particular row or object.

  Renders through `Kati.Components.MishkaMenu`, because a context menu and a
  ...
  (`Kati.Components.Anchored`) does draw a panel over the page against a
  trigger's own box, and `Kati.Components.MishkaPopover` is built on it.
  This renders through `Kati.Components.MishkaMenu`, which has not been
  ...
  Everything `Kati.Components.MishkaMenu` accepts, plus:
