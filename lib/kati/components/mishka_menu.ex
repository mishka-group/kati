defmodule Kati.Components.MishkaMenu do
  @moduledoc """
  Native Mob port of Mishka Chelekom's **headless Menu** — a list of actions
  revealed from a trigger.

  Builds on `Kati.Components.MishkaPopover.panel/2`, so a menu and a popover
  ...
  dropdown, or inside a `Kati.Components.MishkaDrawer` for a bottom-sheet
  ...
  (`Kati.Components.Anchored`). The menu has not been moved onto it, and it
  ...
  | plus everything `Kati.Components.MishkaPopover` accepts | | | |
  ...
  overlays do exist now — `Kati.Components.Anchored`, which `popover`,
  ...
  alias Kati.Components.{Event, MishkaPopover}
