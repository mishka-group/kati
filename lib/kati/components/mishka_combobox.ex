defmodule Kati.Components.MishkaCombobox do
  @moduledoc """
  ...
  `Kati.Components.MishkaSelect`'s job, so the option list and the
  ...
  estimate `Kati.Components.MishkaTagsInput` uses — nothing here can measure
  text.
  ...
  `:anchored` node (`Kati.Components.Anchored`), which
  `Kati.Components.MishkaPopover` is built on — but its window dismisses
  ...
  alias Kati.Components.{Event, MishkaMenu, MishkaPill, MishkaSelect, MishkaTagsInput}
  ...
  make a list unsearchable. An empty query matches everything.
      ...
      iex> alias Kati.Components.MishkaCombobox, as: C
  ...
  Case-fold and strip diacritics, so "Café" and "cafe" compare equal.
      ...
      iex> Kati.Components.MishkaCombobox.fold("Café")
      "cafe"
      iex> Kati.Components.MishkaCombobox.fold(nil)
      ""
  """
