defmodule Kati.Components.MishkaAlertDialog do
  @moduledoc """
  ...
  `Kati.Components.MishkaDialog` — same overlay, same anatomy, same part
  ...
  *something* there is what makes it modal.
  ...
  The first three are `Kati.Components.MishkaDialog`'s own tags, not
  ...
  `Kati.Components.MishkaDialog` for the full table — they are deliberately
  ...
  Everything `Kati.Components.MishkaDialog` accepts, except `dismissible`
  ...
  (`MishkaDialog.trigger/3` has it), `on_open_change` (it fires on backdrop
  ...
  alias Kati.Components.{Event, MishkaDialog}
  ...
  defp warn_no_heading(props) do
    if is_nil(Map.get(props, :title)) and is_nil(Map.get(props, :description)) do
      Logger.warning(
        "[MishkaAlertDialog] open with neither title nor description — the user is being " <>
