defmodule Kati.Components.MishkaFloatingWindow do
  @moduledoc """
  ...
  **itself** — so the drag surface is one STATIC canvas the size of the whole
  ...
  finger took hold, so the window does not jump to centre itself under it:
      ...
      iex> alias Kati.Components.MishkaFloatingWindow
  ...
  easy to get backwards.
      ...
      iex> Kati.Components.MishkaFloatingWindow.nudge(%{x: 40, y: 40, step: 10}, :up)
      {40, 30}

      iex> Kati.Components.MishkaFloatingWindow.nudge(%{x: 40, y: 40, step: 10}, :right)
      {50, 40}
  ...
  against the far edge of the stage rather than walking half out of view:
      ...
      iex> Kati.Components.MishkaFloatingWindow.nudge(
      ...>   %{x: 90, y: 0, step: 20, width: 120, height: 80, bounds: {200, 200}},
      ...>   :right
      ...> )
      {80, 0}

      iex> Kati.Components.MishkaFloatingWindow.nudge(%{x: 0, y: 0, bounds: {200, 200}}, :left)
      {0, 0}
  """
  ...
  text a device test can query, so this is the only handle it has on them.
      ...
      iex> Kati.Components.MishkaFloatingWindow.part_id("win", :close)
      "win-close"

      iex> Kati.Components.MishkaFloatingWindow.part_id("win", {:nudge, :up})
      "win-nudge-up"

      iex> Kati.Components.MishkaFloatingWindow.part_id(nil, :close)
      nil
  """
  ...
  test cannot read a colour.
      ...
      iex> Kati.Components.MishkaFloatingWindow.handle_id("win", false)
      "win-handle"

      iex> Kati.Components.MishkaFloatingWindow.handle_id("win", true)
      "win-handle-dragging"
  """
  ...
  when there is one. The body starts here, and spends the rest of `height`.
      ...
      iex> Kati.Components.MishkaFloatingWindow.chrome_height(%{})
      48

      iex> Kati.Components.MishkaFloatingWindow.chrome_height(%{on_move: :move})
      96
  """
