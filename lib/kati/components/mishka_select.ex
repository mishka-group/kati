defmodule Kati.Components.MishkaSelect do
  @moduledoc """
  ...
  positioning, and this used to say Mob does not have any. It does: the
  `:anchored` node (`Kati.Components.Anchored`) draws a panel in its own
  window, against the anchor's own bounds and clipped by no ancestor, and
  `Kati.Components.MishkaPopover` is built on it.

  A select still renders its list in flow beneath the trigger, and the reason is
  the list rather than the primitive: a list of options is as long as its data,
  and one floating over a phone covers the field it came from. For a long list,
  put the same options inside a bottom `Kati.Components.MishkaDrawer`,
  ...
  `side` is dropped because a list in flow has no side to take, and
  `highlight_on_hover` because a touch screen still has no hover.
  ...
  `highlight_on_hover` (the list is in flow, and a touch screen has no hover)
  and the `*_class` attrs.
  """

  import Mob.Sigil

  alias Kati.Components.{Event, MishkaMenu}
  ...
  the order rather than having it sorted underneath them.
      ...
      iex> alias Kati.Components.MishkaSelect
  ...
  a multi-select that closed after every pick would be exhausting.
      ...
      iex> Kati.Components.MishkaSelect.toggle(nil, :a, false)
      {:a, true}
      iex> Kati.Components.MishkaSelect.toggle(:a, :a, false)
      {:a, true}
      iex> Kati.Components.MishkaSelect.toggle([:a], :b, true)
      {[:a, :b], false}
      iex> Kati.Components.MishkaSelect.toggle([:a, :b], :a, true)
      {[:b], false}
  """
  ...
  The text a trigger shows for a value.
      ...
      iex> Kati.Components.MishkaSelect.display(nil, [], "Pick one")
      "Pick one"
      iex> Kati.Components.MishkaSelect.display(:a, [{:a, "Alpha"}], "Pick one")
      "Alpha"
      iex> Kati.Components.MishkaSelect.display([:a, :b], [{:a, "Alpha"}, {:b, "Beta"}], "x")
      "Alpha, Beta"
      iex> Kati.Components.MishkaSelect.display([], [{:a, "Alpha"}], "Pick one")
      "Pick one"
  """
