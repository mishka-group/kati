defmodule Kati.Components.MishkaJsonInput do
  @moduledoc """
  ...
  it is a question about *one field*. This is the handle for asking it.
      ...
      iex> Kati.Components.MishkaJsonInput.error_id("config")
      "config-error"
  """
  ...
  and the web component does not paint one red either.
      ...
      iex> Kati.Components.MishkaJsonInput.validate(~s({"a": 1}))
      {:ok, %{"a" => 1}}

      iex> Kati.Components.MishkaJsonInput.validate("   ")
      :empty

      iex> match?({:error, _}, Kati.Components.MishkaJsonInput.validate("{oops}"))
      true

  A bare scalar is valid JSON, and treating it as an error would be wrong:

      iex> Kati.Components.MishkaJsonInput.validate("42")
      {:ok, 42}
  """
  ...
  reformatting as someone types would move the cursor out from under them.
      ...
      iex> Kati.Components.MishkaJsonInput.format(~s({"a":1}))
      "{\\n  \\"a\\": 1\\n}"

      iex> Kati.Components.MishkaJsonInput.format("{oops}")
      "{oops}"
  """
  ...
  otherwise the parse decides.
      ...
      iex> Kati.Components.MishkaJsonInput.invalid?(%{}, {:error, "nope"})
      true

      iex> Kati.Components.MishkaJsonInput.invalid?(%{}, :empty)
      false

      iex> Kati.Components.MishkaJsonInput.invalid?(%{invalid: true}, {:ok, 1})
      true
  """
