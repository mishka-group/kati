defmodule Kati.Components.MishkaTagsInput do
  @moduledoc """
  ...
  Composes `Kati.Components.MishkaPill` for the tokens (each with its own
  ...
  | `wrap_chars` | number | `34` | Roughly how much text fits on one token row. |
  ...
  character budget — an estimate, but a MEASURED one. On a phone, inside a card:
  the row is 263dp wide, a character at `:base` is 7.6dp, and a token's padding
  plus its ✕ plus the gap to the next one costs 41dp. In units of one character
  that is a budget of 34, with each token costing its length plus 5.

  Two earlier guesses were wrong in both directions — 28 broke "United Kingdom"
  onto a line of its own, and 40 packed a third token into a row that fits two.
  The second is the worse failure, so when in doubt this UNDER-packs: a sparse
  row merely looks sparse, while an over-packed one squeezes its last token
  until the text wraps character by character. Raise it for a wider control,
  lower it for a narrow one.
  ...
  alias Kati.Components.{Event, MishkaPill}
  ...
  Trims, ignores blanks, and refuses duplicates unless `allow_duplicates: true`.
      ...
      iex> Kati.Components.MishkaTagsInput.add(["a"], "b")
      ["a", "b"]
      iex> Kati.Components.MishkaTagsInput.add(["a"], "  a  ")
      ["a"]
      iex> Kati.Components.MishkaTagsInput.add(["a"], "   ")
      ["a"]
      iex> Kati.Components.MishkaTagsInput.add(["a"], "a", allow_duplicates: true)
      ["a", "a"]
  """
  ...
  Remove a tag.
      ...
      iex> Kati.Components.MishkaTagsInput.remove(["a", "b"], "a")
      ["b"]
  """
  ...
  a row to itself rather than being dropped.
      ...
      iex> Kati.Components.MishkaTagsInput.wrap(["a", "b"], 28)
      [["a", "b"]]
      iex> Kati.Components.MishkaTagsInput.wrap(["aaaaaaaa", "bbbbbbbb", "cccccccc"], 20)
      [["aaaaaaaa"], ["bbbbbbbb"], ["cccccccc"]]
      iex> Kati.Components.MishkaTagsInput.wrap([], 28)
      []
  """
