defmodule Kati.Components.MishkaPreviewCard do
  @moduledoc """
  ...
  `Kati.Components.MishkaPopover`, so holding the trigger a second time can
  ...
  The pair is an `:anchored` node now (`Kati.Components.Anchored`). Child
  ...
  sharing a parent with it:
    ...
    * The card hugs rather than filling. Inside a popup window "fill" is the
      ...
      `Kati.Components.MishkaPopover.panel/2` honours an explicit value.
  ...
  | plus everything `Kati.Components.MishkaPopover`'s **panel** accepts | | | `width`, `background`, `corner_radius`, `padding`, `border_*`, `fill_width`. |
  ...
  alias Kati.Components.{Anchored, Event, MishkaAvatar, MishkaPopover}
