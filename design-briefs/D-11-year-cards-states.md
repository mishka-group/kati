...
Show the five non-dark conditions the share flow has to survive, so none of them reads as a bug.

## Where it is reached from

Not a pushed screen — a states reference sheet beside 63, following the precedent of screen 27 ("Empty, loading, offline").

## Bands, top to bottom

1. Not enough data — literal copy "Not much to show yet — Kati has 3 weeks of your year."
2. Partial year — August, pixel field roughly 60% empty, drawn as deliberate rather than broken
3. A title marked private — the poster collage replaces it with a paper-filled slot; count line reads "1 hidden"
4. Rendering — skeleton, never a spinner: #E7E3DC block plus the #E7E3DC→#F1EEE9→#E7E3DC shimmer bars, in the card's shape
5. Save failed / not supported yet — an honest failure card for the case where file sharing has not landed, plus what the user can do instead (that alternative is not specified by the ticket)

## States to draw

- not enough data
- partial year
- hidden private title
- rendering skeleton
- save unsupported / failed

## Reuse, do not invent

Screen 27's empty state, skeleton row and error-card-with-Retry recipes; pixel field (partially filled); poster stack with an empty slot; elevated card; mono caption.

## Left open — decide and note which way you went

The ticket explicitly leaves the not-enough-data case open: "Draw a card that is still worth saving in that state, or say plainly that there is nothing yet." The save-failure card's recovery advice is also unspecified.
