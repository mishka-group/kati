...
Proves a search result never truncates the very substring the user searched for, by re-laying the result rows at Dynamic Type 235%.

## Where it is reached from

Not navigable — accessibility reference for screen 19, the way screen 41 is for the densest card. Screen 19 is reached from screen 01's search field.

## Bands, top to bottom

1. Back pill '‹ Home' at grown type
2. Search field carrying a long query that wraps rather than clips
3. Scope chip row at 235% — ticket does not say whether the row still scrolls horizontally or wraps to two lines at this size
4. Result rows re-laid as stacks: thumbnail/icon tile above, title wrapping to as many lines as it needs with the matched substring still emphasised, context line, chevron; cards get taller, nothing truncates
5. A long-title row proving the emphasised substring survives the reflow intact

## States to draw

- 235% Dynamic Type, LTR (the ticket does not ask for a 235% Persian variant)

## Reuse, do not invent

Screen 41's 235% reflow rules (rows become stacks, icon-only buttons grow labels, nothing truncates); list row; poster tile; filter/scope chip.

## Left open — decide and note which way you went

Whether the scope chip row scrolls or wraps at 235%. Whether the group cap stays at three rows when each row is three times taller.
