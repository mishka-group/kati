...
Write a single weight reading into the Weight screen.

## Where it is reached from

The new Weight screen — the ticket says only that an entry modal exists "in D-02's geometry" and does not name the control that opens it.

## Bands, top to bottom

1. Modal chrome per D-02: content-height bottom sheet, 26 px top corners, centred title, leading `close` glyph, no trailing Save, no back pill, no tab bar
2. Weight number field as a large DM Mono hero numeral with steppers on either side (D-02's field recipe) — the ticket specifies the geometry, not the field set
3. Unit — kg / lb / st, tied to screen 54's Content → units; the ticket names the unit switch but not its placement in the modal
4. Date of the reading — not specified by the ticket
5. Ink primary commit button — label not specified
...
D-02's modal chrome (matching 06 and 18); DM Mono hero numeral with steppers; segmented control; primary ink button; undo bar from 27 on return

## Left open — decide and note which way you went

The entire field list beyond the number is unspecified — the ticket only borrows D-02's geometry. Whether the modal also carries the unit switch or inherits it silently from 54.
