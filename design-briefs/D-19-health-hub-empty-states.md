# Health hub — empty states

> **Reference sheet** · issue [#22](https://github.com/mishka-group/kati/issues/22) · ticket `D-19`

Show what the Health hub looks like when nothing behind it exists — the state of every user who did not pick Health at onboarding, and the state when meals are switched off.

## Where it is reached from

Variants of screen 42 Health hub, itself reached from screen 01 Home's Sections grid. Confirmed in 42.html.

## Bands, top to bottom

1. "Nothing in Health set up" — no meals, no weight, nothing; the hub with no data at all
2. "Meals off" — the eaten-today ring and macro split bar are meal-derived, so the hub has no content; draw what remains
3. Built vs retired tiles shown visually distinct, retired ones in a clearly labelled group or absent when their section is off

## States to draw

- Nothing in Health set up
- Meals off
- Dark
...
Screen 27's empty-state geometry; screen 42's tile grid; macro bar; progress bar; info footnote; screen 27's new sixth band treatment

## Left open — decide and note which way you went

Whether these are separate artboards or extra bands on the redrawn 42. Whether a retired tile disappears entirely when its section is off in screen 24, or stays visible in the labelled group. Copy for the nothing-set-up state is not given.
