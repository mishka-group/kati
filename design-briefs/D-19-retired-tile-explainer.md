# Retired tile explainer

> **Modal sheet** · issue [#22](https://github.com/mishka-group/kati/issues/22) · ticket `D-19`

Explain, in Kati's voice, why a drawn-but-unbuilt surface is not in this version when its tile is tapped.

## Where it is reached from

Screen 42's retired tiles (Sleep, Workouts, and Medication if retired); the same treatment then covers the greyed Books/Music shelves on 03/57 and screen 36's "Browser extension — Not installed" row.

## Bands, top to bottom

1. Name of the retired feature
2. Short explanation of why it is not in this version — Kati's voice, explicitly no fake "coming soon" date; the ticket gives no copy
3. Dismiss — the ticket says tapping must do "a short explanation, not nothing" and specifies no further affordance
...
Modal chrome (centred title, leading `close`); info footnote card; screen 42's existing "Not set up" tile treatment as the entry affordance

## Left open — decide and note which way you went

Whether this is its own artboard or is drawn inside screen 27's new sixth band, which the acceptance requires to specify "what tapping it does". All copy is unwritten. Whether a retired tile is hidden entirely when its section is off in screen 24's Sections list — the ticket raises this and leaves it open.
