...
> **Modal sheet** · issue [#13](https://github.com/mishka-group/kati/issues/13) · ticket `D-10`

Lets the user pick the country that decides what 'available' means, from a searchable list.

## Where it is reached from

The REGION row at the top of the new My services screen (tapping it opens this picker).

## Bands, top to bottom

1. Header — ticket does not say modal or pushed; if modal, centred title with leading close glyph; if pushed, back pill '‹ My services' (title string not specified)
2. Search field
3. Country rows — flag glyph or tile + country name, with the current selection ticked
4. Search-with-no-match state for the country list (ticket specifies no-match copy only for the SERVICE search, not this one)
5. Footnote repeating that availability data is per-country

## States to draw

- default list with current country ticked
- search in progress with a query typed

## Reuse, do not invent

Modal header with leading close (design-index.md:179) or back pill (:180) depending on the decision; list row (:184); mono section label (:183); info footnote (:203); the search field pattern from screens 01/19.

## Left open — decide and note which way you went

Modal versus pushed screen — the ticket says only 'opens a searchable country picker'. Title string. No-match copy for the country search. Whether an A-Z index or recently-used group appears.
