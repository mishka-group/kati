...
| `Kati.Notifications.Pending` | `:internal` | Kati's own record of what it believes is armed on **this device's** platform. Every row is a claim about an OS-level alarm a restored phone has never held, and the whole set is recomputed from the domains on the next foreground anyway — `Kati.Notifications.Reconcile` reconciles rather than accumulates, so carrying it would restore a belief about a platform rather than a fact about the user. |
