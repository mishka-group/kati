...
| 75 | 92 My services | `open, half done` | Home and screen 92 give opposite answers one tap apart. On a phone with no services Home's row reads "No subscriptions yet"; tapping it opens 92 listing Lumen+ £8.99, Orbit £13.99, Kino £11.49, "Subscribed · 3", and "£46.47 A MONTH". **Half done 6 September (44445c1):** 92 renders from ONE map read in `load/1`, so `Kati.ScreenDesignLiteralTest.drawn_state/0` can now put it in the state its own board draws — that was the blocking half. **The other half of that shape note is wrong, and this is what reading board 93 showed:** 93 cannot BE 92's empty state. It has no way to add a service — 92's *Something else* row is not on it — and its *Free with ads* group lists Aria Free and Dispatch, two fixture services a reader with nothing has not got. A device sent there could see it had nothing and do nothing about it, which is worse than the disagreement. What 92's empty state should be — 92's own chrome over an empty list, in the manner screen 03 keeps over screen 27's card, with *Something else* still on it — is a drawing that does not exist. Needs a board, or a decision to build it without one. |
