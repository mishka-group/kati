...
| 106 | 12 Lists | `code done · design filed` | ~~Screen 12 has one working control and it writes nothing.~~ Everything the store can answer shipped 7 September; the rest is filed as [mishka-group/kati#99](https://github.com/mishka-group/kati/issues/99) because it needs a **drawing**, not a column. Shipped: `Rewatches` and `Abandoned` are the reader's own counts — one query each, the finding's own *cheapest real rows on the page* — and `Wishlist` and `Owned on disc` are not drawn at all, because both are assertions a reader makes about a title and no column holds; a card where two rows count the real library and two are somebody else's reads as fully real, which is #75's call one screen over. The four dead chevrons are gone. `+` no longer prepends a row titled `New list` that was lost on back and duplicated on a second press — it says in one line what it is waiting for. Blocked, and each is an undrawn surface rather than a schema question: **nowhere to name a list** (board 12 has no text entry, and a list created as `New list` with no rename is the same lie further in), **no list picker** for board 146's *Add to list*, and **no list detail** behind the chevrons. The issue carries the two-table schema those drawings would need so design does not have to guess at what is cheap. Not guessed at here, for `Kati.Screens.ImportSources`'s stated reason: three undrawn surfaces is three inventions. Walked on the Pixel_9a — **Abandoned · 1** against a real dropped title, and `+` says what it needs. |
